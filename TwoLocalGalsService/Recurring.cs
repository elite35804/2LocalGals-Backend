    ...
    class Recurring
    {
        ...
        public static void RunEmailPayroll()
        {
            try
            {
                ...
                foreach (ContractorStruct contractor in Database.GetContractorList(-1, false, true, false, false, "lastName, firstName"))
                {
        ...
        public static void RunEmailSchedules()
        {
            try
            {
                ...
                foreach (ContractorStruct contractor in Database.GetContractorList(-1, false, true, true, false, "lastName, firstName"))
                {
        ...
        public static void RunBatches()
        {
            try
            {
                ...
                foreach (FranchiseStruct fran in Database.GetFranchiseList())
                {
                    ...
                    string error = ECreditCard.CloseBatch(fran.ePNAccount, fran.restrictKey);
                    if (error == null)
                    {
        ...
        public static void RunMassEmails()
        {
            try
            {
                ...
                if (DateTime.Now.Hour == 09)
                {
                    ...
                    if (sentList.Count < 250)
                    {
                        foreach (DBRow massEmail in Database.GetMassEmailsToSend())
                        {
                            DBRow setRow = new DBRow();
                            int massEmailID = massEmail.GetInt("massEmailID");
                            int status = 1;
                            CustomerStruct customer;
                            string error = Database.GetCustomerByID(-1, massEmail.GetInt("customerID"), out customer);
                            if (error != null)
                            {
