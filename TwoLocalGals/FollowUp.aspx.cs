...
namespace TwoLocalGals
{
    public partial class FollowUp : System.Web.UI.Page
    {
        ...
        private void LoadAppointment()
        {
            try
            {
                int appID = Globals.SafeIntParse(Request["appID"]);
                if (appID <= 0) appID = Globals.SafeIntParse(Globals.Decrypt(Request["appID"]));
                
                AppStruct app;
                string error = Database.GetApointmentpByID(-1, appID, out app);
